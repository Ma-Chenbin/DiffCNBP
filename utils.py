...
# Adapted from https://github.com/caogang/wgan-gp/blob/master/gan_toy.py
def calc_gradient_penalty_with_gt_v2(net_dis, real_data, fake_data, batch_size, lmbda, use_cuda=False, real_gt=None,
                                     fake_gt=None):
    # Compute interpolation factors
    alpha = torch.rand(batch_size, 1, 1)
    alpha = alpha.expand(real_data.size())
...
def get_plots(true_bp, fake_bp):
    # print(true_bp.shape)
    # for i in range(num_of_samples):
    fig, axs = plt.subplots(2)
    # sns_plot = sns.lineplot(x = [i for i in range(len(true_bp))], y=true_bp, ax=axs[0])
    # sns_plot = sns.lineplot(x = [i for i in range(len(fake_bp))], y=fake_bp, ax=axs[2])
    # print("true bp shape = ", true_bp.shape)
    # print("fake bp shape =", fake_bp.shape)
    true_bp_8_chs = true_bp.reshape(8, 512)
    fake_bp_8_chs = fake_bp.reshape(8, 512)

    for i in range(8):
        sns_plot = sns.lineplot(x=[i for i in range(len(true_bp_8_chs[i]))], y=true_bp_8_chs[i], ax=axs[0])
        sns_plot = sns.lineplot(x=[i for i in range(len(fake_bp_8_chs[i]))], y=fake_bp_8_chs[i], ax=axs[1])
...
def get_plots_RHTM(true_bp, fake_bp):
    # print(true_bp.shape)
    # for i in range(num_of_samples):
    fig, axs = plt.subplots(2)
    # sns_plot = sns.lineplot(x = [i for i in range(len(true_bp))], y=true_bp, ax=axs[0])
    ...
    # print("true bp shape = ", true_bp.shape)
    # print("fake bp shape =", fake_bp.shape)
    true_bp_8_chs = true_bp.reshape(8, 4096)
    fake_bp_8_chs = fake_bp.reshape(8, 4096)
...
def get_plots_RHTM_10s(true_bp, fake_bp):
    # print(true_bp.shape)
    # for i in range(num_of_samples):
    fig, axs = plt.subplots(2)
    # sns_plot = sns.lineplot(x = [i for i in range(len(true_bp))], y=true_bp, ax=axs[0])
    ...
    # print("true bp shape = ", true_bp.shape)
    # print("fake bp shape =", fake_bp.shape)
    true_bp_8_chs = true_bp.reshape(8, 5000)
    fake_bp_8_chs = fake_bp.reshape(8, 5000)
...
def get_plots_all(true_bps, fake_bps, num_of_plots=4):
    # print(true_bp.shape)
    # for i in range(num_of_samples):
    bs = true_bps.shape[0]
    if bs > num_of_plots:
    ...
    for b in range(bs):
        ...
        true_bp_8_chs = true_bps[b].reshape(8, 512)
        fake_bp_8_chs = fake_bps[b].reshape(8, 512)

        for i in range(8):
            sns_plot = sns.lineplot(x=[i for i in range(len(true_bp_8_chs[i]))], y=true_bp_8_chs[i], ax=axs[b, 0])
            sns_plot = sns.lineplot(x=[i for i in range(len(fake_bp_8_chs[i]))], y=fake_bp_8_chs[i], ax=axs[b, 1])
...
def get_plots_all_RHTM(true_bps, fake_bps, num_of_plots=4):
    # print(true_bp.shape)
    # for i in range(num_of_samples):
    bs = true_bps.shape[0]
    if bs > num_of_plots:
    ...
    for b in range(bs):
        ...
        true_bp_8_chs = true_bps[b].reshape(8, 4096)
        fake_bp_8_chs = fake_bps[b].reshape(8, 4096)
...
def get_plots_all_RHTM_10s(true_bps, fake_bps, num_of_plots=4):
    # print(true_bp.shape)
    # for i in range(num_of_samples):
    bs = true_bps.shape[0]
    if bs > num_of_plots:
    ...
    for b in range(bs):
        ...
        true_bp_8_chs = true_bps[b].reshape(8, 5000)
        fake_bp_8_chs = fake_bps[b].reshape(8, 5000)
