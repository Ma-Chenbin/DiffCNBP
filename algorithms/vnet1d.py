...
class Vnet1dCore(nn.Module):
    def __init__(self, n_channels, f_kernel_size, f_out_ch, f_padding,
        ...
        self.init_layer = nn.Sequential(*[
            nn.Conv1d(n_channels, f_out_ch, kernel_size=f_kernel_size, stride=1, padding=f_padding,
        ...
        for l in range(self.num_levels):
            self.down_layers.append(
                DownConvBlock(out_ch, kernel_size, num_convolutions[l], dropout_rate, factor, verbose))
        ...
        for l in reversed(range(self.num_levels)):
            self.up_layers.append(
                UpConvBlock(out_ch, kernel_size, num_convolutions[l], dropout_rate, factor, up_pad, up_out_pad,
                            verbose))
...
class DownConvBlock(nn.Module):
    def __init__(self, n_channels, kernel_size, n_convs, dropout, factor, verbose=True):
        ...
        self.down_conv = nn.Sequential(*[
            nn.Conv1d(n_channels, n_channels * factor, kernel_size=kernel_size, stride=factor,
...
class UpConvBlock(nn.Module):
    def __init__(self, n_channels, kernel_size, n_convs, dropout, factor, pad, out_pad, verbose=True):
        ...
        self.up_conv = nn.Sequential(*[
            nn.ConvTranspose1d(n_channels, n_channels // factor, kernel_size, stride=factor, padding=pad,
