...
class ViT(nn.Module):
    def __init__(self, *, sequence_len, num_patches, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', dim_head=64, dropout = 0., emb_dropout = 0.):
        super().__init__()
        assert pool in {'cls', 'mean'}, 'pool type must be either cls (cls token) or mean (mean pooling)'  #mean
        ...
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout)  # Transformer
    ...
    def forward(self, data):
        ...
        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b = b)  # shape (b, 1, dim)
