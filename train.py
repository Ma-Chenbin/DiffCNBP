...
# https://github.com/vlbthambawita/Pulse2Pulse
# ===========================================
...
from ppg_dataloader import PPGDataSimple as ppg_data
from algorithms.pulse2pulse import WaveGANGenerator as Pulse2PuseGenerator
from algorithms.pulse2pulse import WaveGANDiscriminator as Pulse2PulseDiscriminator
from utils import calc_gradient_penalty, get_plots_RHTM_10s, get_plots_all_RHTM_10s
...
# ==============================
parser.add_argument("--data_dirs", default=["/home/vajira/DL/Pulse2Pulse/sample_ppg_data",
...
# ==========================================
def prepare_data():
    dataset = ppg_data(opt.data_dirs, norm_num=6000, cropping=None, transform=None)
...
def train(netG, netD, optimizerG, optimizerD, dataloader):
    for epoch in tqdm(range(opt.start_epoch + 1, opt.start_epoch + opt.num_epochs + 1)):
        ...
        for i, sample in tqdm(enumerate(dataloader, 0)):
            ...
            real_ppgs = sample["ppg_signals"].to(device)
            # print("real ppgs shape", real_ppgs.shape)
            b_size = real_ppgs.size(0)
            ...
            gradient_penalty = calc_gradient_penalty(netD, real_ppgs,
            ...
            if train_G_flag:
                ...
                # print("real ppg:", real_ppgs.shape)
                # print("fake ppg:", fake.shape)
            if i == 0:  # take the first batch to plot
                real_ppgs_to_plot = real_ppgs
                fake_to_plot = fake
            #    break
        # print(G_cost_epoch)
        ...
        if epoch % opt.checkpoint_interval == 0:
            ...
            fig = get_plots_RHTM_10s(real_ppgs_to_plot[0].detach().cpu(), fake_to_plot[0].detach().cpu())
            fig_2 = get_plots_all_RHTM_10s(real_ppgs_to_plot.detach().cpu(), fake_to_plot.detach().cpu())
