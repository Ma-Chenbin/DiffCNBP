...
# ====== Only for testing ========================
# data_root = "/home/vajira/DL/ppg_test/PPG_data/medians"
# gt_csv = "/home/vajira/DL/ppg_test/PPG_data/ground_truth.csv"
# columns_to_return_gt = ["QOnset", "QOffset", "POnset", "POffset", "TOffset"]

data_csv = [  # "/home/vajira/ppg/GE/asc/ground_truth_updated_vajira.csv",
    "/home/vajira/ppg/gesus/asc/ground_truth_retired190424_updated.csv",
    "/home/vajira/ppg/int99/asc/ground_truth_retired190424_updated.csv"]
data_root = [  # "/home/vajira/ppg/GE/asc/medians",
    "/home/vajira/ppg/gesus/asc/medians", "/home/vajira/ppg/int99/asc/medians"]
columns_to_return = ["P_RInterval", "QRSDuration", "Q_TInterval", "QOnset", "QOffset", "POnset", "POffset", "TOffset"]


class PPGDataALL(Dataset):
    def __init__(self, csv_files, root_dirs, columns_to_return_gt, transform=None):
        ...
        self.df_gt = self.df_gt.loc[
            (self.df_gt["PPGCategory"] == "Normal PPG")]  # | (self.df_gt["PPGCategory"] == "Otherwise normal PPG")]
    ...
    def __getitem__(self, idx):
        ...
        ppg_signals = pd.read_csv(asc_file_name, header=None, sep=" ")  # read into dataframe
        ppg_signals = torch.tensor(ppg_signals.values)  # convert dataframe values to tensor

        # normalization
        ppg_signals = ppg_signals.float()
        # print(ppg_signals)

        # print(ppg_signals.shape)
        ppg_signals = ppg_signals / 6000

        # cropping
        ppg_signals = ppg_signals  # [0:4096, :]

        # Transposing the ppg signals
        ppg_signals = ppg_signals.t()
        # ppg_signals = ppg_signals.unsqueeze(0)

        # reshaping to 1 x 4800 tensor
        # ppg_signals = torch.reshape(ppg_signals, (-1,))

        # print("float ppg data")
        # print(ppg_signals)
        ...
        sample = {'ppg_signals': ppg_signals}

        if self.transform:
            pass
            # sample = self.transform(sample["ppg_signals"])

        return sample


class PPGDataSimple(Dataset):
    def __init__(self, data_dirs, norm_num=6000, cropping=None, transform=None):
        ...
        for data_dir in data_dirs:
            ppg_files = glob.glob(data_dir + "/*")
            self.all_ppg_files = self.all_ppg_files + ppg_files

        self.transform = transform

    def __len__(self):
        return len(self.all_ppg_files)

    def __getitem__(self, idx):

        # Get file name of asc file
        asc_file_name = self.all_ppg_files[idx]
        ...
        # print(ppg_signals.shape)
        ppg_signals = ppg_signals / self.norm_num  # Normalizing aplitude of voltage levels

        # cropping
        if self.cropping:
            ppg_signals = ppg_signals[self.cropping[0]:self.cropping[1], :]
        ...
        if self.transform:
            ppg_signals = self.transform(ppg_signals)
            # sample = self.transform(sample["ppg_signals"])
...
if __name__ == "__main__":
    data_roots = [  # "/home/vajira/ppg/GE/asc/medians",
        "/home/vajira/ppg/gesus/asc/rhythm",
        "/home/vajira/ppg/int99/asc/rhythm"
    ]
    gt_csvs = [  # "/home/vajira/ppg/GE/asc/ground_truth.csv",
        "/home/vajira/ppg/gesus/asc/ground_truth.csv",
        "/home/vajira/ppg/int99/asc/ground_truth.csv"
    ]

    gt_csvs_new = [  # "/home/vajira/ppg/GE/asc/ground_truth.csv",
    ...
    dataset = PPGDataALL(gt_csvs_new, data_roots, columns_to_return_gt)
    ...
    t = dataset[1]
    print(t)
    print("ppg shape:", t["ppg_signals"].shape)
